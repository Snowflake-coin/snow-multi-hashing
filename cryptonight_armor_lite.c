...
static void do_armor_lite_blake_hash(const void* input, size_t len, char* output) {
    blake256_hash((uint8_t*)output, input, len);
}

void do_armor_lite_groestl_hash(const void* input, size_t len, char* output) {
    groestl(input, len * 8, (uint8_t*)output);
}

static void do_armor_lite_jh_hash(const void* input, size_t len, char* output) {
    int r = jh_hash(HASH_SIZE * 8, input, 8 * len, (uint8_t*)output);
    assert(SUCCESS == r);
}

static void do_armor_lite_skein_hash(const void* input, size_t len, char* output) {
    int r = c_skein_hash(8 * HASH_SIZE, input, 8 * len, (uint8_t*)output);
    assert(SKEIN_SUCCESS == r);
}

static void (* const extra_hashes[4])(const void *, size_t, char *) = {
    do_armor_lite_blake_hash, do_armor_lite_groestl_hash, do_armor_lite_jh_hash, do_armor_lite_skein_hash
};
...
struct cryptonightarmorlite_ctx {
...
#if defined(_MSC_VER)
    struct cryptonightarmorlite_ctx *ctx = _malloca(sizeof(struct cryptonightarmorlite_ctx));
#else
    struct cryptonightarmorlite_ctx *ctx = alloca(sizeof(struct cryptonightarmorlite_ctx));
#endif
