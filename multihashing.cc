...
DECLARE_FUNC(chukwasnow) {
    DECLARE_SCOPE;

    // Chukwa Snow Definitions
    const uint32_t hashlen = 32;    // The length of the resulting hash in bytes
    const uint32_t saltlen = 16;    // The length of our salt in bytes
    const uint32_t threads = 1;     // How many threads to use at once
    const uint32_t iters = 16;      // How many iterations we perform as part of our slow-hash
...
DECLARE_INIT(init) {
    ...
    NODE_SET_METHOD(exports, "chukwa-snow", chukwasnow);
